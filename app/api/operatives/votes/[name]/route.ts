...
export async function POST(
  ...
  try {
    ...
    await castCharacterOptionVote(sanitizedName, "op", vote, user);
    return NextResponse.json(
      { message: "Vote cast successfully" },
      { status: 200 }
    );
  } catch (error) {
    console.error("Error processing vote for operative", error);
    return NextResponse.json(
      { error: "Failed to process vote for operative" },
      { status: 500 }
    );
  }
}
