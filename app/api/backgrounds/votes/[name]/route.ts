...
export async function POST(
  ...
  try {
    ...
    await castCharacterOptionVote(sanitizedName, "bg", vote, user);
    return NextResponse.json(
      { message: "Vote cast successfully" },
      { status: 200 }
    );
  } catch (error) {
    console.error("Error processing vote for background", error);
    return NextResponse.json(
      { error: "Failed to process vote for background" },
      { status: 500 }
    );
  }
}
