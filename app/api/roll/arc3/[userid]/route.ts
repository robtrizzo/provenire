...
import { RollArc3, validateRollArc3 } from "@/types/roll";
import { checkAuth } from "@/lib/auth";
import { addRollArc3, getRollsArc3 } from "@/handlers/rolls";
...
export async function POST(
  ...
  const roll = (await request.json()) as RollArc3;
  if (!roll) {
    return new NextResponse(null, { status: 400 });
  }
