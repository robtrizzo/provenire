...
export default function CharacterOptionVotePopover({ name }: { name: string }) {
...
function VoteSection({ name }: { name: string }) {
  ...
  const { pointsSpent, refetchVotes, votesFetching } = useOperatives();
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      ...
      <TypographyP>
        Your points remaining: <b>{5 - pointsSpent}</b>
      </TypographyP>
