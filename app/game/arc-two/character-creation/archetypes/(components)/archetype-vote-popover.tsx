...
import { useArchetypes } from "@/contexts/archetypesContext";
...
export default function ArchetypeVotePopover({ name }: { name: string }) {
  ...
  if (!session.data?.user) {
    return (
      <div className="flex flex-col items-center gap-2">
        ...
        <TypographyP>
          You must be logged in to cast a vote for this archetype.
        </TypographyP>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      ...
      <span className="text-xs">
        Select your interest level in this archetype.
      </span>
      <TypographyP>
...
function VoteTypeSection({
  ...
  const { votes, castVote, votesFetching } = useArchetypes();

  const archetypeVotes = votes.find((vote) => vote.name === name)?.votes[vote];
  ...
  return (
    <div
      ...
      onClick={() => {
        ...
        castVote({ archetype: name, vote });
      }}
    >
      <VoteTypeSectionDescriptor vote={vote} />
      <div className="flex flex-col gap-1">
        {archetypeVotes?.map((user) => (
