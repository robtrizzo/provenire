...
export default function DiceSheet() {
    ...
    const fetchRollData = async ({ pageParam = 0, queryKey} :{pageParam: number | undefined, queryKey: string[]}) => {
    ...
    return (
        <Sheet open={open} onOpenChange={setOpen}>
            ...
            <SheetContent className="max-h-screen overflow-y-scroll">
                ...
                {usersArePending ? <div className="flex items-center justify-center h-screen"><Loader className="animate-spin" /></div> :
                    <div className="flex items-center space-x-4">
                        <Select onValueChange={(value) => {handleValueChange(value)}}>
                            ...
                            <SelectContent>
                                ...
                                {users?.filter((user: User) => user.id !== session?.data?.user?.id)
                                    .map((user: User) =>
                ...
                <div className="overflow-auto">
                    {rollsArePending || !selectedValue ? <div className="flex items-center justify-center h-screen"><Loader className="animate-spin" /></div> :
                    <DiceHistory
                            rolls={rolls}
                            rollsUntilNearEnd={5}
